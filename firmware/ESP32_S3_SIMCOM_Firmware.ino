/*
 * VGU Thesis
 * Project: ESP32-S3 Firmware (SIMCOM A7682S 4G + Flash Storage)
*/
...
void setup() {
  Serial.begin(115200);
  Serial.println("\n--- SYSTEM START ---");
  Serial.println("MCU Name: ESP32_N16R8 (Custom Board)");
  ...
  if (!needs_time_sync || now.year() >= 2024) {
    current_timestamp = now.unixtime() - 25200;  // VN offset timestamp
  }
  ...
  // 1. Save immediate reading to Flash
