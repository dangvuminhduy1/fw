/*
 * VGU Thesis
 * Project: ESP32-S3 Firmware (Flash Storage + Potentiometers) 
*/
...
#define MAX_OFFLINE_READINGS 300       // Up to 5 hours of 1-minute offline data
...
float readSalinityFromPot() {
  uint32_t mv = 0;
  for (int i = 0; i < 20; i++) {
    mv += analogReadMilliVolts(SAL_POT_PIN);
    delay(1);
  }
  float averageMv = mv / 20.0;
  // Map 0 - 3245mV to an arbitrary 0 - 100 ppt range "tượng trưng"
  float salValue = (averageMv / 3245.0) * 100.0;
  if (salValue < 0) salValue = 0.0;
  if (salValue > 100.0) salValue = 100.0;
  return salValue;
}
...
// =================================
// FLASH STORAGE & REALTIME DATABASE
// =================================
...
bool processAndSendQueue() {
  ...
  if (count > 0) {
    ...
    // Change .POST to .sendRequest("PATCH", ...)
